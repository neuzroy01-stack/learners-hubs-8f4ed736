...
// Helper function to resolve video sources (Google Drive, YouTube, Direct MP4)
const getVideoSource = (rawUrl: string | null | undefined) => {
  ...
  // 1. Google Drive Links (Extracts File ID and converts to preview player)
  ...
  // 2. YouTube Links (Supports watch?v=, youtu.be/, shorts/, embed/)
...
export const VideoPlayerView: React.FC<VideoPlayerViewProps> = ({ course, onBack }) => {
  ...
  // Resolve current active video source from Database URL
            ...
            {videoSource.type === 'direct' ? (
              ...
              <>
                <iframe
                  ...
                  allowFullScreen
                />
                {/* Google Drive Protect Overlay: Blocks top-right pop-out button */}
                {videoSource.type === 'google-drive' && (
                  <div
                    className="absolute top-0 right-0 w-20 h-16 z-20 bg-transparent cursor-default pointer-events-auto"
                    title="External playback disabled"
                  />
                )}
              </>
            ) : (
