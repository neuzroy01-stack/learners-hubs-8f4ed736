...
import { CheckCircle2, XCircle, Trash2, RefreshCw, Pencil, ShieldCheck } from 'lucide-react';
import { paymentsApi, pendingPaymentsWithNames } from '../../services/cloudDb';
...
export const PaymentApprovalView: React.FC = () => {
  ...
  const act = async (id: string, status: 'verified' | 'rejected') => {
    try {
      ...
      notify('success', status === 'verified' ? 'Payment approved' : 'Payment rejected', 'Student balance recalculated.');
      await reload();
    } catch (err) {
        ...
        <table className="w-full min-w-[720px] text-left text-xs">
          <thead className="bg-slate-50 text-[10px] uppercase text-slate-500 dark:bg-slate-800">
            <tr>
              ...
              <th className="p-3">Method</th><th className="p-3">Reference / UTR</th><th className="p-3">Date</th><th className="p-3">Actions</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((p) => (
              <tr key={p.id} className="border-t border-slate-100 dark:border-slate-800">
                ...
                <td className="p-3">
                  <div className="flex gap-1.5">
                    <button onClick={() => void act(p.id, 'verified')} className="rounded-lg bg-emerald-600 p-1.5 text-white" aria-label="Approve"><CheckCircle2 className="h-3.5 w-3.5" /></button>
                    <button onClick={() => void act(p.id, 'rejected')} className="rounded-lg bg-amber-500 p-1.5 text-white" aria-label="Reject"><XCircle className="h-3.5 w-3.5" /></button>
            ...
            {rows.length === 0 && !loading && (
              <tr><td colSpan={7} className="p-8 text-center text-slate-500">No payments awaiting verification.</td></tr>
            )}
          </tbody>
        </table>
      </div>
